...
writes into your own workspace.
...
```
<workspace>/memory/sources.json    what is connected, and how fresh each one is
<workspace>/memory/records/*.md    one memory per file, sharing policy in the frontmatter
```
