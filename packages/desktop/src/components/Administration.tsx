...
/** Running a workspace, as a dialog. The panel below is the same thing inline. */
 ...
 *
 * Kept separate from the dialog shell so Settings can host the very same
 * surface inline — there should be one place that runs a workspace, not one in
 * a modal and a thinner copy under Settings.
 */
export function AdministrationPanel({
  ...
  workspace: WorkspaceView;
  onMessage: (address: string) => void;
  /** Called when an action here means the caller should get out of the way. */
  onClose?: () => void;
  initialTab?: Tab;
}) {
...
function MembersTable({
  ...
  workspace: WorkspaceView;
  onMessage: (address: string) => void;
  /** Absent when this table is inline rather than in a dialog. */
  onClose?: () => void;
}) {
