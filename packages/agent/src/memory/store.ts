 ...
 * from.
 *
 *   <knowledge-base>/memory/sources.json   what is connected and how fresh it is
 *   <knowledge-base>/memory/records/*.md   one memory per file, policy in frontmatter
 *
...
export class MemoryIndex extends EventEmitter {
  ...
  /** `root` is the knowledge base directory; the index lives in `memory/` under it. */
