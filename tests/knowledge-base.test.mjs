...
import { KnowledgeBase, findPersona, seedKnowledgeBase } from '../packages/agent/dist/index.js';
...
test('two knowledge bases never share state', async (t) => {
  const dirA = await makeTempDir();
  const dirB = await makeTempDir();
  t.after(() => cleanup([dirA, dirB]));

  const a = await KnowledgeBase.open(dirA);
  const b = await KnowledgeBase.open(dirB);
  await seedKnowledgeBase(a, findPersona('sarah'));
  await seedKnowledgeBase(b, findPersona('tom'));

  // Regression: a shallow-copied empty db once made every fresh knowledge base share
...
test('a knowledge base round-trips through disk', async (t) => {
  const dir = await makeTempDir();
  t.after(() => cleanup([dir]));

  const first = await KnowledgeBase.open(dir);
  await seedKnowledgeBase(first, findPersona('priya'));
...
test('private material never reaches a meeting digest', async (t) => {
  ...
  await seedKnowledgeBase(ws, findPersona('dana'));
