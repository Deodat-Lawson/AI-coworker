...
import { Vault, KnowledgeBase } from '../packages/agent/dist/index.js';
import {
  buildBacklinks,
  buildGraph,
  buildResolver,
...
test('the agent reads hand-written vault notes, folders and all', async (t) => {
  ...
  const titles = knowledge.notes.map((n) => n.title).sort();
  assert.deepEqual(titles, ['Private thought', 'Rollout plan']);

  const rollout = knowledge.notes.find((n) => n.title === 'Rollout plan');
  assert.equal(rollout.path, 'Projects/Rollout.md');
  assert.ok(rollout.tags.includes('release'));

  const priv = knowledge.notes.find((n) => n.title === 'Private thought');
  ...
  await knowledge.upsertNote({ id: priv.id, title: priv.title, body: 'Revised.', kind: 'idea' });
  ...
  const reopened = await KnowledgeBase.open(dir);
...
test('a note the agent writes lands in the vault as a plain markdown file', async (t) => {
  ...
  const knowledge = await KnowledgeBase.open(dir);
  await knowledge.upsertNote({ title: 'Decision: use refresh tokens', body: 'Because sessions leak.' });
  await knowledge.flush();
